...
public class FileIO implements Debuggable, Serializable {
	...
	public Automobile readCarConfig(String fileName) {
		...
		// int numOfProperties = 0; // number of OptionSets to be initialized
		// String[] optionTitles; // holds names of each OptionSet
		// String[] metaData; // holds metadata from line 1
		String name = ""; // 1
		String basePrice = ""; // 2
		...
		try {
			...
			while (counter != (num OfProperties + 2)) {
				...
				else if (counter == 1) // OptionSetNames
				{
					...
					for (int i = 0; i < numOfProperties; i++) { // initializes OptionSets and assigns their names
						...
						opset[i].setChoiceType(optionTitles[i]);
					}
					counter++;
				}
				...
				else if (counter > 1) // optionSet choices
				{
					...
					opset[(counter - 2)].buildChoiceArray(choiceName, price); // creates array of Options inside
		...
		auto = new Automobile(name, opset, basePrice); // name, price, OptionSets passed to Automobile constructor
		if (DEBUG)
